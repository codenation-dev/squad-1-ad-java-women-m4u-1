...
class LogTypeTypeController {
    ...
    @GetMapping("/errorcenter/logTypes")
    ...
    @GetMapping("/errorcenter/logTypes/{id}")
    ...
    @PostMapping("/errorcenter/logTypes")
    ...
    @ApiOperation(value = "Altera um logType")
    @ApiResponses({
            @ApiResponse(code = 200, message = "LogType criado com sucesso", response = LogTypeDTO.class),
            @ApiResponse(code = 404, message = "LogType não encontrado", response = MessageDTO.class)
    })
    @PutMapping("/errorcenter/logTypes/{id}")
    LogType updateLogType(@RequestBody LogTypeDTO logAlterada, @PathVariable Long id) {
        return logTypeServiceInterface.update(new ModelMapper()
                .map(logAlterada, LogType.class), id)
                .orElseThrow(() -> { throw new LogTypeNotFoundException(id); });
    }
    ...
    @DeleteMapping("/errorcenter/logTypes/{id}")
